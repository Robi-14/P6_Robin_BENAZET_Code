...
*/
      case 1 :
     Sauces.updateOne({ _id : sauceId},{ $push :{ usersLiked : user}, $inc:{ likes:+1}})
...
//  si like = -1 l'utilisateur dislike la sauce , utilisation des opérateurs de mise à jour mongodb
     ...
     Sauces.updateOne({_id : sauceId}, {$push:{usersDisliked: user}, $inc:{dislikes:+1}})
     ...
     case 0:
       ...
       .then((sauce)=> {
        if (sauce.usersLiked.includes(user)){
          Sauces.updateOne({_id : sauceId}, {$pull:{usersLiked: user}, $inc:{likes:-1}})
          .then(() => res.status(200).json({ message: 'Sauce neutre !'}))
          .catch(error => res.status(400).json({ error }))
        } else if(sauce.usersDisliked.includes(user)){
          Sauces.updateOne({_id : sauceId}, {$pull:{usersDisliked: user}, $inc:{dislikes:-1}})
          .then(() => res.status(200).json({ message: 'Sauce neutre !'}))
          .catch(error => res.status(400).json({ error }))
